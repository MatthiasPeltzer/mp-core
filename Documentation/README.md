# MP Core Documentation

Documentation hub for the MP Core TYPO3 13 / 14 site package.

---

## Getting Started

- **[Installation & Quick Start](../README.md)** -- Get up and running in minutes
- **[Feature Overview](OVERVIEW.md)** -- All capabilities at a glance

## Guides

- [Frontend](Frontend.md) -- Build system, Vite, JavaScript/SCSS architecture
- [Configuration](Configuration.md) -- Site Sets, Site Settings, TypoScript, TCA
- [Favicons](Favicons.md) -- Favicon generation pipeline

## External Resources

- [TYPO3 Documentation](https://docs.typo3.org/)
- [Bootstrap 5](https://getbootstrap.com/docs/5.3/)
- [Vue.js 3](https://vuejs.org/)
- [Vite](https://vitejs.dev/)
- [Swiper](https://swiperjs.com/)
- [b13/container](https://github.com/b13/container)
