import { r as __exportAll } from "./rolldown-runtime-CL6CfywC.js";
//#region node_modules/jarallax/dist/jarallax.esm.js
var jarallax_esm_exports = /* @__PURE__ */ __exportAll({ jarallax: () => jarallax });
/*!
...
var win$1;
if (typeof window !== "undefined") win$1 = window;
else if (typeof global !== "undefined") win$1 = global;
else if (typeof self !== "undefined") win$1 = self;
else win$1 = {};
var global$2 = win$1;
function css(el, styles) {
	if (typeof styles === "string") return global$2.getComputedStyle(el).getPropertyValue(styles);
...
function getParents(elem) {
	...
	while (currentElement.parentElement !== null) {
		...
		if (currentElement.nodeType === 1) parents.push(currentElement);
	}
	return parents;
}
...
var mobileAgent = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(global$2.navigator?.userAgent ?? "");
function isMobile() {
	return mobileAgent;
}
var wndW = 0;
var wndH = 0;
var deviceHelper = null;
function hasDocument() {
	return typeof document !== "undefined" && typeof document.documentElement !== "undefined";
}
function getDeviceHeight() {
	if (!hasDocument()) return global$2.innerHeight || 0;
...
var visibilityObserver = global$2.IntersectionObserver && new global$2.IntersectionObserver((entries) => {
	entries.forEach((entry) => {
		const target = entry.target;
		if (target.jarallax) target.jarallax.isElementInViewport = entry.isIntersecting;
	});
}, { rootMargin: "50px" });
function addObserver(instance) {
	jarallaxList.push({ instance });
	if (jarallaxList.length === 1) global$2.requestAnimationFrame(updateParallax);
	visibilityObserver?.observe(instance.options.elementInViewport || instance.$item);
}
function removeObserver(instance) {
	jarallaxList.forEach((data, key) => {
		if (data.instance.instanceID === instance.instanceID) jarallaxList.splice(key, 1);
	});
	visibilityObserver?.unobserve(instance.options.elementInViewport || instance.$item);
}
var globalNavigator = global$2.navigator ?? { userAgent: "" };
var canUseDOM = typeof document !== "undefined" && typeof Element !== "undefined" && typeof HTMLElement !== "undefined";
var instanceID = 0;
...
var Jarallax = class {
	constructor(item, userOptions) {
		...
		Object.keys(dataOptions).forEach((key) => {
			...
			if (lowerCaseOption && typeof this.defaults[lowerCaseOption] !== "undefined") pureDataOptions[lowerCaseOption] = dataOptions[key];
		});
		...
		if (elementInVP && typeof elementInVP === "object" && "length" in elementInVP && !(elementInVP instanceof Element)) [elementInVP] = Array.from(elementInVP);
		...
		if (this.initImg() && this.canInitParallax()) this.init();
	}
	css(el, styles) {
	...
	initImg() {
		...
		if (imageElement && typeof imageElement === "string") imageElement = this.$item.querySelector(imageElement);
		if (!(imageElement instanceof Element)) if (this.options.imgSrc) {
			const image = new Image();
			image.src = this.options.imgSrc;
			imageElement = image;
		} else imageElement = null;
		if (imageElement) {
			if (this.options.keepImg) this.image.$item = imageElement.cloneNode(true);
			else {
		...
		if (this.image.$item) return true;
	...
	init() {
		...
		if (!this.options.keepImg) {
			...
			if (itemStyle) this.$item.setAttribute("data-jarallax-original-styles", itemStyle);
			if (this.image.useImgTag && this.image.$item) {
				const imageStyle = this.image.$item.getAttribute("style");
				if (imageStyle) this.image.$item.setAttribute("data-jarallax-original-styles", imageStyle);
			}
		}
		if (this.css(this.$item, "position") === "static") this.css(this.$item, { position: "relative" });
		if (this.css(this.$item, "z-index") === "auto") this.css(this.$item, { zIndex: 0 });
		this.image.$container = document.createElement("div");
		this.css(this.image.$container, containerStyles);
		this.css(this.image.$container, { zIndex: this.options.zIndex });
		if (this.image.position === "fixed") this.css(this.image.$container, {
			WebkitClipPath: "polygon(0 0, 100% 0, 100% 100%, 0 100%)",
			clipPath: "polygon(0 0, 100% 0, 100% 100%, 0 100%)"
		});
		this.image.$container.setAttribute("id", `jarallax-container-${this.instanceID}`);
		if (this.options.containerClass) this.image.$container.setAttribute("class", this.options.containerClass);
		this.$item.appendChild(this.image.$container);
		if (this.image.useImgTag && this.image.$item) imageStyles = this.extend({
			...
			if (this.image.src) imageStyles = this.extend({
		...
		if (this.options.type === "opacity" || this.options.type === "scale" || this.options.type === "scale-opacity" || this.options.speed === 1) this.image.position = "absolute";
		if (this.image.position === "fixed") {
			const parents = getParents(this.$item).filter((el) => {
				...
				return parentTransform && parentTransform !== "none" || /(auto|scroll)/.test(styles.overflow + styles.overflowY + styles.overflowX);
			});
		...
		if (this.css(this.$item, "background-image") !== "none") this.css(this.$item, { backgroundImage: "none" });
		addObserver(this);
	}
	destroy() {
		removeObserver(this);
		...
		if (!originalStylesTag) this.$item.removeAttribute("style");
		else this.$item.setAttribute("style", originalStylesTag);
		if (this.image.useImgTag && this.image.$item) {
			...
			if (!originalStylesImgTag) this.image.$item.removeAttribute("style");
			else this.image.$item.setAttribute("style", originalStylesImgTag);
			this.image.$itemParent?.appendChild(this.image.$item);
		}
		if (this.image.$container?.parentNode) this.image.$container.parentNode.removeChild(this.image.$container);
	...
	onScroll(force) {
		...
		if (this.options.type === "scale" || this.options.type === "scale-opacity") {
			let scale = 1;
			if (this.options.speed < 0) scale -= this.options.speed * visiblePercent;
			else scale += this.options.speed * (1 - visiblePercent);
		...
		if (this.options.type === "scroll" || this.options.type === "scroll-opacity") {
			...
			if (this.image.position === "absolute") positionY -= contT;
...
var jarallax$1 = function jarallax2(items, options, ...args) {
	let normalizedItems = items;
	if (!items) return items;
	if (typeof HTMLElement === "object" ? items instanceof HTMLElement : items && typeof items === "object" && items.nodeType === 1 && typeof items.nodeName === "string") normalizedItems = [items];
	if (!normalizedItems || typeof normalizedItems !== "object" || typeof normalizedItems.length !== "number") return normalizedItems;
	if (!canUseDOM) return typeof options === "string" ? void 0 : normalizedItems;
	let ret;
	for (let index = 0; index < normalizedItems.length; index += 1) {
		const item = normalizedItems[index];
		if (typeof options === "object" || typeof options === "undefined") {
			if (!item.jarallax) item.jarallax = new Jarallax(item, options);
		} else if (item.jarallax) ret = item.jarallax[options].apply(item.jarallax, args);
...
function extend(out, ...args) {
	...
	Object.keys(args).forEach((index) => {
		...
		Object.keys(source).forEach((key) => {
			...
			if (typeof value !== "undefined") target[typedKey] = value;
		});
	});
	return target;
}
...
var VideoWorkerBase = class {
	...
	on(name, callback) {
		...
		if (!this.userEventsList[name]) this.userEventsList[name] = [];
	...
	off(name, callback) {
		...
		this.userEventsList[name].forEach((value, key) => {
			...
			if (value === callback) {
				...
				if (eventList) eventList[key] = false;
			}
		});
	}
	fire(name, ...args) {
		if (this.userEventsList && typeof this.userEventsList[name] !== "undefined") this.userEventsList[name].forEach((value) => {
			if (value) value.apply(this, args);
		});
	}
	/**
	...
	*/
	...
	destroy() {
		...
		if ((_a = this.$video) == null ? void 0 : _a.parentNode) this.$video.parentNode.removeChild(this.$video);
		if ((_b = this.hiddenContainer) == null ? void 0 : _b.parentNode) this.hiddenContainer.parentNode.removeChild(this.hiddenContainer);
		this.player = void 0;
		this.$video = void 0;
...
var VideoWorkerLocal = class extends VideoWorkerBase {
	...
	play(start) {
		...
		if (typeof start !== "undefined") this.player.currentTime = start;
		if (this.player.paused) if (this.options.endTime && !this.options.loop) this.getCurrentTime((seconds) => {
			var _a;
			if (seconds < this.options.endTime) (_a = this.player) == null || _a.play();
		});
	...
	setVolume(volume = false) {
		if (!this.player || !this.$video || typeof volume !== "number") return;
	...
	setCurrentTime(currentTime = false) {
		if (!this.player || !this.$video || typeof currentTime !== "number") return;
	...
	getImageURL(callback) {
		if (this.videoImage) callback(this.videoImage);
	}
	getVideo(callback) {
		...
		if (!this.$video && hiddenDiv) {
			...
			if (this.options.showControls) this.$video.controls = true;
			if (typeof this.options.volume === "number") this.setVolume(this.options.volume);
			...
			Object.keys(this.videoID).forEach((key) => {
				...
				if (sourceValue) addSourceElement(this.$video, sourceValue, `video/${key}`);
			});
			const player = this.player;
			let localStarted = false;
			this.eventHandlers = {
				playing: (event) => {
					if (!localStarted) this.fire("started", event);
					localStarted = true;
				},
				timeupdate: (event) => {
					this.fire("timeupdate", event);
					if (this.options.endTime && this.player && this.player.currentTime >= this.options.endTime) if (this.options.loop) this.play(this.options.startTime);
					else this.pause();
				},
				play: (event) => {
				...
				loadedmetadata: (event) => {
					...
					if (this.options.autoplay) this.play(this.options.startTime);
				},
				volumechange: (event) => {
...
var videoGlobal$1 = global$1;
function loadAPI$1() {
	if (VimeoAPIadded) return;
	VimeoAPIadded = 1;
	if (typeof videoGlobal$1.Vimeo !== "undefined") return;
...
function onAPIready$1(callback) {
	...
	} else if (typeof videoGlobal$1.Vimeo !== "undefined") callback();
...
var VideoWorkerVimeo = class extends VideoWorkerBase {
	...
	static parseURL(url) {
		const match = url.match(/https?:\/\/(?:www\.|player\.)?vimeo.com\/(?:channels\/(?:\w+\/)?|groups\/([^/]*)\/videos\/|album\/(\d+)\/video\/|video\/|)(\d+)(?:$|\/|\?)/);
	...
	play(start) {
		if (!this.player) return;
		if (typeof start !== "undefined") this.player.setCurrentTime(start);
		this.player.getPaused().then((paused) => {
			var _a;
			if (paused) if (this.options.endTime && !this.options.loop) this.getCurrentTime((seconds) => {
				var _a2;
				if (seconds < this.options.endTime) (_a2 = this.player) == null || _a2.play();
			});
			else (_a = this.player) == null || _a.play();
		});
	}
	pause() {
		if (!this.player) return;
		this.player.getPaused().then((paused) => {
			var _a;
			if (!paused) (_a = this.player) == null || _a.pause();
		});
	}
	mute() {
	...
	setVolume(volume = false) {
		if (!this.player || typeof volume !== "number" || !this.player.setVolume) return;
	...
	getVolume(callback) {
		...
		if (this.player.getVolume) this.player.getVolume().then((volume) => {
			callback(volume * 100);
		});
	}
	getMuted(callback) {
	...
	setCurrentTime(currentTime = false) {
		if (!this.player || typeof currentTime !== "number" || !this.player.setCurrentTime) return;
	...
	getImageURL(callback) {
		...
		if (global$1.devicePixelRatio) width *= global$1.devicePixelRatio;
	...
	getVideo(callback) {
		...
		onAPIready$1(() => {
			...
			if (urlHash) this.playerOptions.h = urlHash;
			if (!this.options.showControls) this.playerOptions.controls = 0;
			if (!this.options.showControls && this.options.loop && this.options.autoplay) this.playerOptions.background = 1;
			if (!this.$video && hiddenDiv) {
				this.hiddenContainer = hiddenDiv;
			...
			if (!this.options.mute && typeof this.options.volume === "number") this.setVolume(this.options.volume);
			if (this.options.startTime && this.options.autoplay) this.player.setCurrentTime(this.options.startTime);
			...
			this.player.on("timeupdate", (event) => {
				...
				this.fire("timeupdate", event);
				if (this.options.endTime && event.seconds >= this.options.endTime) if (this.options.loop) this.play(this.options.startTime);
				else this.pause();
			});
			this.player.on("play", (event) => {
				this.fire("play", event);
				if (this.options.startTime && event.seconds === 0) this.play(this.options.startTime);
			});
	...
	destroy() {
		...
		if ((_a = this.player) == null ? void 0 : _a.destroy) this.player.destroy();
		super.destroy();
	}
};
...
var videoGlobal = global$1;
function loadAPI() {
	if (YoutubeAPIadded) return;
	YoutubeAPIadded = 1;
...
function onAPIready(callback) {
	...
	} else if (typeof videoGlobal.YT === "object" && videoGlobal.YT.loaded === 1) callback();
...
var VideoWorkerYoutube = class extends VideoWorkerBase {
	...
	static parseURL(url) {
		const match = url.match(/.*(?:youtu.be\/|v\/|u\/\w\/|embed\/|shorts\/|watch\?v=)([^#&?]*).*/);
	...
	play(start) {
		if (!this.player || !this.player.playVideo || !videoGlobal.YT) return;
		if (typeof start !== "undefined") this.player.seekTo(start || 0);
		if (videoGlobal.YT.PlayerState.PLAYING !== this.player.getPlayerState()) if (this.options.endTime && !this.options.loop) this.getCurrentTime((seconds) => {
			var _a;
			if (seconds < this.options.endTime) (_a = this.player) == null || _a.playVideo();
		});
		else this.player.playVideo();
	}
	pause() {
		if (!this.player || !this.player.pauseVideo || !videoGlobal.YT) return;
		if (videoGlobal.YT.PlayerState.PLAYING === this.player.getPlayerState()) this.player.pauseVideo();
	}
	mute() {
	...
	getVolume(callback) {
		...
		if (this.player.getVolume) callback(this.player.getVolume());
	}
	getMuted(callback) {
		if (!this.player) {
			callback(null);
		...
		if (this.player.isMuted) callback(this.player.isMuted());
	}
	setCurrentTime(currentTime = false) {
		if (!this.player || typeof currentTime !== "number" || !this.player.seekTo) return;
	...
	getVideo(callback) {
		...
		onAPIready(() => {
			...
			this.playerOptions = {
				...
				events: {
					onReady: (event) => {
						if (this.options.mute) event.target.mute();
						else if (typeof this.options.volume === "number") event.target.setVolume(this.options.volume);
						if (this.options.autoplay) this.play(this.options.startTime);
						this.fire("ready", event);
						if (this.options.loop && !this.options.endTime && this.player) {
							const secondsOffset = .1;
							this.options.endTime = this.player.getDuration() - secondsOffset;
						}
						if (this.volumeChangeInterval) clearInterval(this.volumeChangeInterval);
			...
			this.playerOptions.events.onStateChange = (event) => {
				if (!videoGlobal.YT || !this.player) return;
				if (this.options.loop && event.data === videoGlobal.YT.PlayerState.ENDED) this.play(this.options.startTime);
				if (!ytStarted && event.data === videoGlobal.YT.PlayerState.PLAYING) {
					ytStarted = true;
					this.fire("started", event);
				}
				if (event.data === videoGlobal.YT.PlayerState.PLAYING) this.fire("play", event);
				if (event.data === videoGlobal.YT.PlayerState.PAUSED) this.fire("pause", event);
				if (event.data === videoGlobal.YT.PlayerState.ENDED) this.fire("ended", event);
				if (event.data === videoGlobal.YT.PlayerState.PLAYING) {
					if (this.progressInterval) clearInterval(this.progressInterval);
					this.progressInterval = setInterval(() => {
						if (!this.player) return;
						this.fire("timeupdate", event);
						if (this.options.endTime && this.player.getCurrentTime() >= this.options.endTime) if (this.options.loop) this.play(this.options.startTime);
						else this.pause();
					}, 150);
...
var VideoWorker = function(url, options) {
	let result = false;
	Object.keys(VideoWorker.providers).forEach((key) => {
		if (!result && VideoWorker.providers[key].parseURL(url)) result = new VideoWorker.providers[key](url, options);
	});
...
//#endregion
export { jarallax_esm_exports as t };

//# sourceMappingURL=vendor-jarallax-BYR86rnq.js.map