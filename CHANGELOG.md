...
### Fixed
- Banner content elements render link text and image again; inline `contentAs` markup no longer uses an invalid `f:section` wrapper that collapsed the bar to a 1px background stripe.

### Changed
- Updated npm dependencies (`@vitejs/plugin-vue`, `autoprefixer`, `postcss-preset-env`, `sass`).

## [1.2.25] - 2026-09-12

### Fixed
- Documentation overview pages show the full intro from `teaser_description` in the page header instead of the truncated `subtitle` field.
- Subpage menu teasers no longer crop `teaser_description` to 200 characters.
...
### Fixed
- Gallery modal prev/next controls show a visible focus ring in dark mode; Bootstrap’s carousel `filter` no longer inverts the white outline to black.
- Carousel controls are excluded from the global link/button focus outline; modal footer prev/next use theme-aware outline colours.
...
### Fixed
- Gallery modal carousel counter (`.carousel-iterator`) is centered again; the `position-absolute` mixin no longer collapses partial `inset` values to all sides.
- Gallery and carousel image links show a visible inset focus ring; gallery Swipers add inner padding so the ring is not clipped by `overflow: hidden`.

### Accessibility
- Gallery `a.popup` links and carousel `.stretched-link` overlays use an inset `box-shadow` focus indicator on keyboard focus.
...
### Changed
- Update frontend build dependencies: swiper 14.0.7, vite 8.2.0, eslint 10.8.0, sass 1.102.0, postcss 8.5.25, globals 17.8.0 and jsdom 30.0.1 (major bump).
...
### Fixed
- Search autosuggest "top results" now link to the correct detail URLs for indexed mediathek entries (uses `static_page_arguments` from the indexer, matching indexed_search result links).
